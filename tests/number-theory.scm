...
(test-group "math.number-theory"
  ...
  (let ()
    ...
    (define (prime-sum start delta pr)
      (define s 0)
      (do ([i start (add1 i)])
          ([= i (+ start delta 1)])
        (when pr
          (print (format "~A: ~A" i (next-prime i)))
          )
        (set! s (+ s (next-prime i))))
      s)
    (test 1001511919 (prime-sum (expt 10 6) 1000 #t))    ; Sum[NextPrime[n], {n, 10^6, 10^6 + 1000}]
    (test 10010514423 (prime-sum (expt 10 7) 1000 #f))   ; Sum[NextPrime[n], {n, 10^7, 10^7 + 1000}]
    (test 100100519271 (prime-sum (expt 10 8) 1000 #f))  ; Sum[NextPrime[n], {n, 10^8, 10^8 + 1000}]
    (test 1001000516807 (prime-sum (expt 10 9) 1000 #f)) ; Sum[NextPrime[n], {n, 10^9, 10^9 + 1000}]
    )
